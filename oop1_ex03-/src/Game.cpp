...
void Game::moveCharacter(direction C) {
    ...
    if (isValidMove(newPos.row, newPos.col)) {
        updateItems(newPos.row, newPos.col);
        if (keylogic(newPos.row, newPos.col))// just for depug
        {
            if (Cathed)
            {
              mouse.reciveIntLocation();
              updateMap(C);
              Cathed = false;
            }
            else {
...
bool Game::keylogic(int x, int y)
{
    
    if (mapSprites[y][x].getTexture() == &textures[5] && key == 0)
    {
...
bool Game::isValidMove(int x, int y) {
    return x >= 0 && x < mapSprites[y].size() && y >= 0 && y < mapSprites.size() &&
        mapSprites[y][x].getTexture() != &textures[0];
...
void Game::updateMap(direction C) {
    ...
    void Game::updateItems(int x, int y)
    {
        if (mapSprites[y][x].getTexture() == &textures[1])
        {
            Cathed = true ; 
        }

        if (mapSprites[y][x].getTexture() == &textures[2])
        {
            CountCheese--;
        } 

        if (mapSprites[y][x].getTexture() == &textures[4])
        {
           ++key;
        }
    }
